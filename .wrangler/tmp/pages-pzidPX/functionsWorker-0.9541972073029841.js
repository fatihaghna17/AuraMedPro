...
var onRequestPost2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (!question || !mode) {
      return new Response(JSON.stringify({ error: "Missing required fields" }), { status: 400, headers: corsHeaders9 });
    }
    ...
    if (clientLimit) {
      if (now > clientLimit.resetAt) {
        ...
        if (clientLimit.count >= RATE_LIMIT_MAX) {
          return new Response(JSON.stringify({ error: "Rate limit exceeded. Coba lagi nanti." }), { status: 429, headers: corsHeaders9 });
        }
        clientLimit.count++;
      }
    } else {
...
Tolong jawab pertanyaan lanjutan tersebut berdasarkan konteks soal di atas.`;
    ...
    for (const model of MODEL_FALLBACK) {
      try {
        ...
        return new Response(JSON.stringify({ explanation: generatedText }), { status: 200, headers: corsHeaders9 });
      } catch (err) {
    ...
    return new Response(JSON.stringify({ error: error.message || "Internal Server Error" }), { status: 500, headers: corsHeaders9 });
  }
}, "onRequestPost");
  ...
  try {
    if (env.R2_SECRET_TOKEN) {
      ...
      if (authHeader !== `Bearer ${env.R2_SECRET_TOKEN}`) {
        return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: corsHeaders9 });
      }
    }
    const { key } = await request.json();
    if (!key) {
      return new Response(JSON.stringify({ error: "Key is required" }), { status: 400, headers: corsHeaders9 });
    }
    await env.QUESTIONS_BUCKET.delete(key);
    return new Response(JSON.stringify({ success: true }), { status: 200, headers: corsHeaders9 });
  } catch (error) {
    return new Response(JSON.stringify({ error: error.message || "Failed to delete" }), { status: 500, headers: corsHeaders9 });
  }
}, "onRequestPost");
...
var onRequestPost11 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (!filename || !content) {
      return new Response(JSON.stringify({ error: "Filename and content are required" }), { status: 400, headers: corsHeaders9 });
    }
    ...
    return new Response(JSON.stringify({ fileUrl, key: sanitizedFilename }), { status: 200, headers: corsHeaders9 });
  } catch (error) {
    return new Response(JSON.stringify({ error: error.message || "Failed to upload" }), { status: 500, headers: corsHeaders9 });
  }
}, "onRequestPost");
...
// ../.wrangler/tmp/pages-pzidPX/functionsRoutes-0.5185522206390933.mjs
var routes = [
  {
